...
.hero-title {
    ...
    text-shadow: 1px 1px 2px rgba(140,82,96,0.3); 
}
.hero-sub {
    font-size: 14px;
    color: #FFFFFF;
...
.metric-card.blue  { border-top: 4px solid #FF8FAB; } 
.metric-card.green { border-top: 4px solid #FFC8DD; } 
.metric-card.amber { border-top: 4px solid #FFAFCC; } 
.metric-card.red   { border-top: 4px solid #BDE0FE; } 
...
with st.sidebar:
    ...
    <div style='padding: 20px 0 16px; text-align: center;'>
        <div style='font-size:26px; font-weight:700; color:#FF8FAB; letter-spacing:-0.5px;'>MCP Optimizer</div>
        <div style='font-size:12px; color:#C0808F; margin-top:4px;'>Material Requirements Planning</div>
    </div>
    """, unsafe_allow_html=True)

    st.markdown('<div class="sidebar-section">Biaya</div>', unsafe_allow_html=True)
    setup_cost    = st.number_input("Setup Cost per Order (S)", value=750, min_value=0)
    holding_cost  = st.number_input("Holding Cost per Unit (H)", value=500, min_value=0)

    st.markdown('<div class="sidebar-section">Inventori</div>', unsafe_allow_html=True)
    initial_inventory = st.number_input("Initial Inventory", value=1000, min_value=0)
    safety_stock      = st.number_input("Safety Stock", value=500, min_value=0)

    st.markdown('<div class="sidebar-section">Waktu</div>', unsafe_allow_html=True)
    lead_time = st.number_input("Lead Time (periods)", value=1, min_value=0)

    st.markdown("""
    <div style='margin-top:32px; padding:16px; background:#FFF9FA; border-radius:12px; border:1px solid #FFEDF1;'>
...
if not required_cols.issubset(df_input.columns):
    st.markdown('<div class="warn-box">⚠️ <strong>Format CSV tidak sesuai.</strong></div>', unsafe_allow_html=True)
    st.stop()
...
with m1:
    st.markdown(f'<div class="metric-card blue"><div class="metric-label">Grand Total Cost</div><div class="metric-value">{fmt_rp_simple(grand_total)}</div><div class="metric-sub">Total biaya keseluruhan</div></div>', unsafe_allow_html=True)
with m2:
    st.markdown(f'<div class="metric-card green"><div class="metric-label">Total Setup Cost</div><div class="metric-value">{fmt_rp_simple(total_setup)}</div><div class="metric-sub">{total_orders} kali order</div></div>', unsafe_allow_html=True)
with m3:
    st.markdown(f'<div class="metric-card amber"><div class="metric-label">Total Holding Cost</div><div class="metric-value">{fmt_rp_simple(total_holding)}</div><div class="metric-sub">{total_units:,} unit dipesan</div></div>', unsafe_allow_html=True)
with m4:
    ratio = (total_holding / grand_total * 100) if grand_total > 0 else 0
    st.markdown(f'<div class="metric-card red"><div class="metric-label">Proporsi Holding</div><div class="metric-value">{ratio:.1f}%</div><div class="metric-sub">dari total biaya</div></div>', unsafe_allow_html=True)


# ─── ITERASI & OPTIMAL ────────────────────────────────────────────────────────
st.markdown('<div class="section-header"><div class="section-icon upload">🔍</div><div><div class="section-title">Proses Iterasi MCP</div></div></div>', unsafe_allow_html=True)

col1, col2 = st.columns([3, 2], gap="large")
with col1:
    st.markdown("**Semua Iterasi yang Diuji**")
    st.dataframe(pd.DataFrame(all_iterations).style.format({"Total Cost (Rp)": "{:,.0f}", "Cost/Period (Rp)": "{:,.1f}", "Net Requirement": "{:,}", "Lot Size": "{:,}"}), use_container_width=True, height=340)
with col2:
    st.markdown("**Kombinasi Optimal per Langkah**")
    st.dataframe(pd.DataFrame(optimal_combinations).style.format({"Total Cost (Rp)": "{:,.0f}", "Cost/Period (Rp)": "{:,.1f}", "Lot Size": "{:,}"}).map(lambda _: "background-color:#FFF0F3; color:#FF8FAB; font-weight:600", subset=["Cost/Period (Rp)"]), use_container_width=True, height=340)

st.markdown("<hr>", unsafe_allow_html=True)


# ─── FINAL MRP SHEET ──────────────────────────────────────────────────────────
st.markdown('<div class="section-header"><div class="section-icon upload">📋</div><div><div class="section-title">Final MRP Sheet</div></div></div>', unsafe_allow_html=True)

mrp_matrix = {"Data / Periode": ["Gross Requirements (GR)", "Scheduled Receipts (SR)", "Projected Available Balance (PAB)", "Net Requirements (NR)", "Planned Order Receipts (PORec)", "Planned Order Releases (PORel)"]}
for k in range(periods):
    lbl = df_input['Period'].iloc[k] if 'Period' in df_input.columns else f"P{k+1}"
    mrp_matrix[lbl] = [mrp_gross_req[k], mrp_scheduled_rec[k], mrp_projected_bal[k], mrp_net_req[k], mrp_planned_receipts[k], mrp_planned_releases[k]]
...
def style_mrp_cute(df):
    ...
    row_colors = {
        "Gross Requirements (GR)":          "background:#FFF9FA; font-weight:600",
        "Projected Available Balance (PAB)": "background:#FFF0F3; color:#FF8FAB",
        "Net Requirements (NR)":             "background:#FFFBEB; color:#92400E; font-weight:600",
        "Planned Order Receipts (PORec)":    "background:#F0FDF4; color:#0F5132; font-weight:600",
        "Planned Order Releases (PORel)":    "background:#FFF7ED; color:#9A3412; font-weight:600",
    }
    for row, style in row_colors.items():
        if row in df.index: styles.loc[row] = style
    return styles

st.dataframe(df_mrp.style.apply(style_mrp_cute, axis=None).format("{:,.0f}"), use_container_width=True, height=260)
st.markdown("<hr>", unsafe_allow_html=True)


# ─── COST BREAKDOWN TABLE ─────────────────────────────────────────────────────
st.markdown('<div class="section-header"><div class="section-icon upload">💰</div><div><div class="section-title">Rincian Biaya per Periode</div></div></div>', unsafe_allow_html=True)

df_cost_display = df_cost.copy()
total_row = {"Periode": "TOTAL", "GR": sum(mrp_gross_req), "Lot Size (PORec)": total_units, "PAB Akhir": "-", "Setup Cost (Rp)": total_setup, "Holding Cost (Rp)": total_holding, "Total Cost (Rp)": grand_total}
...
def style_cost_cute(df):
    ...
    for i in range(len(df)-1):
        if df.iloc[i]["Setup Cost (Rp)"] > 0: styles.iloc[i, df.columns.get_loc("Setup Cost (Rp)")] = "background:#FFF0F3; color:#FF8FAB; font-weight:600"
        if df.iloc[i]["Holding Cost (Rp)"] > 0: styles.iloc[i, df.columns.get_loc("Holding Cost (Rp)")] = "background:#FFFBEB; color:#92400E"
        styles.iloc[i, df.columns.get_loc("Total Cost (Rp)")] = "font-weight:600"
    return styles

st.dataframe(df_cost_display.style.apply(style_cost_cute, axis=None).format({"GR": "{:,.0f}", "Lot Size (PORec)": "{:,.0f}", "Setup Cost (Rp)": "{:,.0f}", "Holding Cost (Rp)": "{:,.0f}", "Total Cost (Rp)": "{:,.0f}"}, na_rep="-"), use_container_width=True, height=460)


# ─── CHARTS ───────────────────────────────────────────────────────────────────
st.markdown('<div class="section-header"><div class="section-icon charts">📈</div><div><div class="section-title">Visualisasi</div></div></div>', unsafe_allow_html=True)
...
with c1:
    fig1 = go.Figure()
    fig1.add_bar(x=chart_labels, y=mrp_gross_req, name="GR", marker_color="#FF99AC")
    fig1.add_bar(x=chart_labels, y=mrp_planned_receipts, name="Lot Size (PORec)", marker_color="#BDE0FE")
    fig1.update_layout(title="GR vs Lot Size per Periode", barmode="group", height=320, legend=dict(orientation="h", y=-0.2), **plotly_layout_base)
    st.plotly_chart(fig1, use_container_width=True)

with c2:
    fig2 = go.Figure()
    fig2.add_bar(x=chart_labels, y=df_cost["Setup Cost (Rp)"].tolist(), name="Setup Cost", marker_color="#FF8FAB")
    fig2.add_bar(x=chart_labels, y=df_cost["Holding Cost (Rp)"].tolist(), name="Holding Cost", marker_color="#C0808F")
    fig2.update_layout(title="Setup vs Holding Cost per Periode", barmode="stack", height=320, legend=dict(orientation="h", y=-0.2), **plotly_layout_base)
    st.plotly_chart(fig2, use_container_width=True)

c3, c4 = st.columns(2, gap="large")
with c3:
    fig3 = go.Figure()
    fig3.add_scatter(x=chart_labels, y=mrp_projected_bal, mode="lines+markers", line=dict(color="#FF8FAB", width=3), marker=dict(size=8), fill="tozeroy", fillcolor="rgba(255,143,171,0.08)")
    fig3.update_layout(title="Projected Available Balance (PAB)", height=320, **plotly_layout_base)
    st.plotly_chart(fig3, use_container_width=True)

with c4:
    if total_setup + total_holding > 0:
        fig4 = go.Figure(go.Pie(labels=["Setup Cost", "Holding Cost"], values=[total_setup, total_holding], hole=0.6, marker=dict(colors=["#FF8FAB", "#C0808F"]), textinfo="label+percent"))
        fig4.update_layout(title="Komposisi Biaya Total", height=320, annotations=[dict(text=fmt_rp_simple(grand_total), x=0.5, y=0.5, font_size=15, showarrow=False, font=dict(color="#FF8FAB", weight="bold"))], **plotly_layout_base)
        st.plotly_chart(fig4, use_container_width=True)

st.markdown(hr_styled, unsafe_allow_html=True)


# ─── DOWNLOAD (FIXED EXCEL GENERATION) ────────────────────────────────────────
st.markdown('<div class="section-header"><div class="section-icon download">📥</div><div><div class="section-title">Download Laporan</div></div></div>', unsafe_allow_html=True)

# Sesi Perbaikan Utama: Memakai openpyxl murni & aman tanpa memanggil xlsxwriter ghaib
df_mrp_dl   = df_mrp.reset_index()
df_cost_dl_full = df_cost_display.copy()

output_excel_buffer = BytesIO()
# Menggunakan engine default openpyxl yang pasti ter-instal di sistem Streamlit
...
st.markdown('<div class="cute-footer">MRP · MCP Optimizer — Material Requirements Planning Dashboard</div>', unsafe_allow_html=True)
