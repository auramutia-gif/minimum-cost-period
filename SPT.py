...
[data-testid="stSidebar"] .stRadio div[role="radiogroup"] > label:hover {
    background: rgba(255, 183, 193, 0.35) !important;
}

/* Active / terpilih — pink pastel solid seperti gambar referensi */
...
.metric-value { font-size: 22px; font-weight: 600; color: #4A2030; font-family: 'DM Mono', monospace; }

/* ─── SECTION HEADER & UTILS ─── */
.section-header { display: flex; align-items: center; gap: 10px; margin: 24px 0 14px; }
.section-title  { font-size: 18px; font-weight: 700; color: #4A2030; }
.info-box { background: #FFF3F5; border-left: 4px solid #FFB7C5; border-radius: 0 10px 10px 0; padding: 13px 16px; margin-bottom: 20px; font-size: 13px; color: #6D404E; }
hr { border-color: #FFE0E7 !important; }
</style>
...
with st.sidebar:
    ...
    <div style='padding: 20px 16px 16px 16px;'>
        <div style='font-size: 20px; font-weight: 700; color: #4A2030; display:flex; align-items:center; gap:8px;'>
            ⏱️ SPT Dashboard
        </div>
        <div style='font-size: 11px; color: #9A6070; margin-top: 3px; letter-spacing: 0.3px;'>
            Shortest Processing Time Optimizer
        </div>
    </div>
    <hr style='margin: 0 16px 12px; border-color: #FCCDD7 !important;'>
    """, unsafe_allow_html=True)
...
# ─── HALAMAN 3: HASIL PENJADWALAN SPT ─────────────────────────────────────────
    ...
    if df_spt is not None:
        ...
        with c3:
            st.markdown(f'<div class="metric-card pastel-green"><div class="metric-label">Urutan Hasil Penjadwalan</div><div class="metric-value" style="font-size:16px;padding-top:4px;">{sequence_str}</div></div>', unsafe_allow_html=True)
    else:
    ...
    if df_spt is not None:
        ...
        for idx, row in df_spt.iterrows():
            ...
            fig_gantt.add_trace(go.Bar(
                ...
                textposition='inside',
                marker=dict(color=pastel_colors[color_idx], line=dict(color='#4A2E35', width=1)),
